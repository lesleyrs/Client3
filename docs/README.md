...
## TODO
```
- GL1.1 issues: forced redraw_background every frame is causing some memleaks EG coins in bank while the interface is open
- GL1.1 optimizations: cache models/pixmaps?, gl transforms, use vertex arrays instead of immediate mode, turn textures into an atlas, use framebuffer textures (not gl 1.1)
