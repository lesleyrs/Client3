...
## TODO
```
GL1.1 issues: make texture shading more accurate, turn textures into a texture atlas for performance? (removes all glBegin(GL_TRIANGLES)/glEnd/glBindTexture) except once in world3d_draw(), hardcoded vertex offsets in pix3d (+ 8 and + 11), needs to be different for model_draw_simple and other pix), pixfont wave/colors broken when drawing pixmaps?
